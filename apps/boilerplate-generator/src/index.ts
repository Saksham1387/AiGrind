...
function generatePartialBoilerplate(generatorFilePath: string) {
  ...
  console.log("Partial-Boilerplate code generated successfully!");
}
...
function main(problemName: string) {
  fs.readdir(process.env.PROBLEMS_DIR_PATH || "", (err, files) => {
    files.forEach(file => {
      if (file === problemName) {
        generatePartialBoilerplate(path.join(process.env.PROBLEMS_DIR_PATH || "", file));
...
// Run the main function with the problem name provided as a command line argument
main(process.argv[2]);